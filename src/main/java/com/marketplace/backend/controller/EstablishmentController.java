...
import com.marketplace.backend.domain.establishment.LoginDTO;
...
public class EstablishmentController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Establishment registered successfully",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = EstablishmentEntity.class))),
            @ApiResponse(responseCode = "400", description = "Invalid establishment data provided", content = @Content)
    })
    @PostMapping("/register")
    public ResponseEntity<EstablishmentEntity> create(@RequestBody EstablishmentEntity establishment) {
        EstablishmentEntity createdEstablishment = establishmentService.create(establishment);
    ...
    @GetMapping("/{uuid}")
    public ResponseEntity<EstablishmentEntity> getById(
        ...
        EstablishmentEntity establishment = establishmentService.getByUuid(uuidConverted);
    ...
    public ResponseEntity<EstablishmentEntity> update(
            @Parameter(description = "UUID of the establishment", required = true) @PathVariable String uuid,
            @RequestBody EstablishmentEntity userDetails) {
        UUID uuidConverted = UUID.fromString(uuid);
        EstablishmentEntity updatedUser = establishmentService.update(uuidConverted, userDetails);
    ...
    @Operation(summary = "Validate establishment password", description = "Validates the password for a establishment identified by their UUID")
    @ApiResponses(value = {
    ...
    public ResponseEntity<Boolean> validatePassword(
        ...
        UUID uuidConverted = UUID.fromString(uuid);
        boolean isValid = establishmentService.validatePassword(uuidConverted, password);
    ...
    @PostMapping("/login")
    public ResponseEntity<EstablishmentEntity> login(@RequestBody LoginDTO loginRequest) {
        EstablishmentEntity establishment = establishmentService.login(loginRequest.getEmail(), loginRequest.getPassword());
        return ResponseEntity.ok(establishment);
    }
}